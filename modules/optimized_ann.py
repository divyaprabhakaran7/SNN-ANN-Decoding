...
class ANN(nn.Module):
    def __init__(self, n_inputs=26, image_size=320):
        ...
        self.PI = torch.tensor(math.pi)  # Create a torch tensor with pi value
    ...
    def _init_weights(self):
        ...
        nn.init.normal_(self.orientation_predictor.weight, std=0.005)
        nn.init.zeros_(self.orientation_predictor.bias)
    
    def _initialize_grid(self, device):
        if not self.grid_initialized:
            # Create meshgrid without indexing parameter for older PyTorch versions
            y = torch.arange(self.image_size, dtype=torch.float32, device=device)
            x = torch.arange(self.image_size, dtype=torch.float32, device=device)
            # Manually create ij indexing behavior
            self.grid_y, self.grid_x = torch.meshgrid(y, x)
            # Transpose to match 'xy' indexing behavior if needed
            self.grid_x = self.grid_x.transpose(0, 1)
            self.grid_y = self.grid_y.transpose(0, 1)
    ...
    def get_target_orientation(self, target):
        ...
        return angle / (2 * self.PI.to(angle.device))
    ...
    def generate_binary_bar(self, centers, angles):
        ...
        for i in range(batch_size):
            ...
            angle_rad = angles[i] * self.PI.to(angles.device)
    ...
    def loss_function(self, pred, target):
        ...
        angle_loss = (
            3.0 * F.mse_loss(torch.cos(pred_angle * 2 * self.PI.to(pred_angle.device)), 
                             torch.cos(target_angle * 2 * self.PI.to(target_angle.device))) +
            F.mse_loss(torch.sin(pred_angle * 2 * self.PI.to(pred_angle.device)), 
                       torch.sin(target_angle * 2 * self.PI.to(target_angle.device)))
        )
        ...
        horizontal_bias = torch.abs(pred_angle).mean()
